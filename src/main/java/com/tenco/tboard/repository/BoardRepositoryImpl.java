...
public class BoardRepositoryImpl implements BoardRepository {
	...
	private static final String SELECT_ALL_BOARDS = " SELECT * FROM board ORDER BY id DESC LIMIT ? OFFSET ? ";
	...
	public Board getBoardById(int id) {
		...
		try (Connection conn = DBUtil.getConnection();
			...
			if (rs.next()) {
				board = Board.builder().id(rs.getInt("id")).user(rs.getInt("user_id")).title(rs.getString("title"))
	...
	public List<Board> getAllBoards(int limit, int offset) {
		...
		try (Connection conn = DBUtil.getConnection();
			...
			while (rs.next()) {
				boardList
						.add(Board.builder().id(rs.getInt("id")).user(rs.getInt("user_id")).title(rs.getString("title"))
								.content(rs.getString("content")).createdAt(rs.getTimestamp("created_at")).build());
			}
