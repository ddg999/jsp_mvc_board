...
<body>
	...
	<c:forEach var="board"  items="${boardList}">
		<div class="board-item">
			...
			<p>${board.content}</p>
			<p><fmt:formatDate value="${board.createdAt}" pattern="yyyy년MM월dd일 HH:mm"/></p>
			<!-- 게시글에 작성자가 세션 유저와 동일하다면 수정, 삭제 버튼을 보여주자  -->
			<c:if test="${board.user == userId}">
				<a class="btn btn-edit" href="${pageContext.request.contextPath}/board/update?boardId=${board.id}">수정</a>
				<a class="btn btn-delete" href="${pageContext.request.contextPath}/board/delete?boardId=${board.id}">삭제</a>
			</c:if>
		</div>
	</c:forEach>
	<br>
